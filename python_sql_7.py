...
from sqlalchemy import insert, select, desc, asc, and_, or_ , distinct, func
...
query_ = query_.where(and_(cookies.c.quantity > 15, cookies.c.unit_cost < 10))
res = conn.execute(query_)

# between
query_ = cookies.select()
query_ = query_.where(cookies.c.quantity.between(10, 30))
res = conn.execute(query_)

query_ = select(func.count(distinct(cookies.c.cookie_name)))
res = conn.execute(query_)
# 
