...
from sqlalchemy import insert, select, desc, asc, and_, or_ , distinct, func, update
...
# update table
query_ = update(cookies).where(cookies.c.cookie_name == 'peanut butter')
query_ = query_.values(quantity = (cookies.c.quantity + 120))
res = conn.execute(query_)

query_ = select(cookies).where(cookies.c.cookie_name == 'peanut butter')
res = conn.execute(query_)
